...
def AgenteExplorador(mundo, mostrar_bfs=False, screen=None):
    # O Agente vai percorrer as casas na ordem em que o BFS visita, passando pelas células/casas usando o menor caminho seguro.
    ...
    # BFS para explorar células seguras e registrar a ordem de visita
    fila = deque()
    fila.append(start)
    visitadas = set([start])
    ordem_bfs = [start]
    ...
    while fila and not achou_ouro:
        ...
        for d, delta in Movimento.items():
            ...
            if 0 <= nx < tamanho and 0 <= ny < tamanho and npos not in visitadas:
                ...
                if (('A' in conteudo) or ('W' in conteudo and mundo.Wumpusvivo)):
                    continue
                visitadas.add(npos)
                ordem_bfs.append(npos)
                fila.append(npos)
        # Mostra o BFS em tempo real no mapa do jogo (coloca os quadrados em azul)
        if mostrar_bfs and screen is not None:
            ...
            pygame.display.flip()
            time.sleep(0.08)
    # Função para encontrar o menor caminho seguro entre duas células
    def CaminhoSeguroBFS(orig, dest):
        fila = deque()
        fila.append((orig, [orig]))
        visit = set([orig])
        while fila:
            ...
            for d in Movimento.values():
                ...
                npos = (nx, ny)
                if 0 <= nx < tamanho and 0 <= ny < tamanho and npos not in abismos and npos not in visit:
                    conteudo = mundo.mapa[nx][ny]
                    if ('A' in conteudo) or ('W' in conteudo and mundo.Wumpusvivo):
                        continue
                    visit.add(npos)
                    fila.append((npos, caminho + [npos]))
        return []
    # Gera as ações para o agente andar na ordem do BFS
    pos = start
    direcao_atual = direcao
    for destino in ordem_bfs[1:]:
        caminho = CaminhoSeguroBFS(pos, destino)
        for passo in caminho[1:]:
            nova_dir = ObterDirecao(pos, passo)
            if direcao_atual != nova_dir:
                acoes.append(('virar', nova_dir))
                direcao_atual = nova_dir
            acoes.append(('mover', nova_dir))
            pos = passo
        if achou_ouro and pos == pos_ouro:
            acoes.append(('pegar', None))
            break
    # Ao final, tenta voltar para a saída
    caminho_saida = CaminhoSeguroBFS(pos, saida)
    for passo in caminho_saida[1:]:
        nova_dir = ObterDirecao(pos, passo)
        if direcao_atual != nova_dir:
            acoes.append(('virar', nova_dir))
            direcao_atual = nova_dir
...
def CaminhoBFS(start, goal, abismos):
    ...
    while fila:
        ...
        for d in Movimento.values():
            nx, ny = pos[0] + d[0], pos[1] + d[1]
            npos = (nx, ny)
            if 0 <= nx < 4 and 0 <= ny < 4 and npos not in abismos and npos not in visitado:
                visitado.add(npos)
                fila.append((npos, caminho + [npos]))
    return []

def ObterDirecao(orig, dest):
    dx = dest[0] - orig[0]
    dy = dest[1] - orig[1]
    if dx == -1:
        return 'N'
...
def main():
    ...
    if modo == 1:
        mostrar_bfs = True
        agente_acoes = AgenteExplorador(mundo, mostrar_bfs=mostrar_bfs, screen=screen)
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
